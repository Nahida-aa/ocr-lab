...
pub fn recognize(
    ...
    tracing::trace!("[rec_in] first24={:?} shape={:?}", rec_in.as_slice().unwrap().iter().take(24).copied().collect::<Vec<f32>>(), rec_in.shape());
    ...
    tracing::trace!(
        "[rec] crop_in={:?} rec_crop={:?} rec_input_w={} -> text={:?} conf={:.4} logits_shape={:?}",
        (img.dim().0, img.dim().1),
        (rec_crop.dim().0, rec_crop.dim().1),
        rec_w,
        text,
        conf,
        shape
