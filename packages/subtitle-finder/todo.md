...
- [ ] **状态机剩余对齐（段 25 vs C++ 22）**
      ...
      - **53033 处 Rust 多一段、52833-53033**（C++ 无）：
        - 帧内容对比：`49967_52800.png` = "毁灭赋能概率实在太小了"（同 C++ 的
          `49966-52832`），`52833_53033.png` = 纯人物背景、无字幕（C++ 在此帧
          仍算有字幕、继续到 52832 结束整段）。
        - Rust 段 END PTS 用末帧真实 PTS（52800），比 C++ `PosForward[offset]-1`
          早 ~32ms（1 帧），导致 52833 帧被检测逻辑误判为「新字幕开始」→ 重新走
          检测循环 → AnalizeImageForSubPresence 仍判 1（残影/人物边）→ 误保存。
        - 修法候选：(a) 让段末 et 用 `PosForward[offset]-1`（向 C++ 靠 32ms，
          但已记录为正向决策，要权衡）；(b) 检测循环对只有 1-3 帧的短段加
          `min_dl` 门槛；(c) AnalizeImageForSubPresence 加白点密度/水平投影
          阈值拒绝纯背景帧。最稳的是 (b) 或 (c)，不动 et 决策。
