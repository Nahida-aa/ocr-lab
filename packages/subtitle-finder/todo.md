...
- [ ] **状态机剩余对齐（段 25 vs C++ 22）**
      ...
      - **53033 处 Rust 多一段、52833-53066**（C++ 无）：
        ...
        - **根因（RUST_LOG=trace 实证）**：帧 1586-1589（pos 52867-52967）与
          1593-1595 有**残影 has_text=1**（isa_wc≈2000-3400），而真实字幕帧
          isa_wc≈13500-14500（4-7× 差距）。这些残影帧被检测循环当成新字幕起点 →
          误保存 52833-53066。
        - 帧 1585(pos 52833) has_text=0、1586 has_text=1(isa_wc=2047) → 字幕刚消失，
          角色边缘（沙发/身体）通过 color_filtration+second_filtration+clear_symbols。
        - 修法候选：(a) has_text 加 isa_wc 阈值（<~4000 拒）；(b) 检测循环对 isa_wc 低
          的短段加门槛；(c) 查 C++ 为何这些帧 has_text=0（Decoder BGR 差异 or
          clear_image_from_small_symbols 差异），对齐 C++ 最本质。
        - 需先确认 C++ 同帧 has_text 是 0（算法/输入差异）还是 1（仅保存路径差异）。
