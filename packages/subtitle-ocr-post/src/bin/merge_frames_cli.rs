...
//! `OcrFramesResult { frames, meta }`），跑 [`subtitle_ocr_post::merge_frames`] 的多段流水线，
//! 把逐帧结果合并成带时间轴的字幕段 [`subtitle_ocr_post::MergeFramesResult`]。结果默认到
...
fn main() -> Result<()> {
    ...
    let result: MergeFramesResult = subtitle_ocr_post::merge_frames(&frames, &args);
