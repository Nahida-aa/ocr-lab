...
1. **视频字幕识别**（核心方向，`subtitle-ocr`）：从视频帧抽取字幕文字。
...
视频字幕识别的基准（`tests/bench/subtitle-ocr`）**只服务字幕识别**，不要与
...
| 路径 | 目标 | 作用 |
| --- | --- | --- |
| `crates/rapidocr-ort` | 1,2,3 | PP-OCR 文字识别（基于 ONNX Runtime）。`det` 检测 + `rec` 识别 + `cls` 方向分类，DB 后处理对齐官方实现。三方向共用的识别引擎，也是视频字幕识别的核心底座。 |
...
| `packages/subtitle-ocr` | 1 | 字幕识别 Rust 实现（**核心方向**，OCR + 后处理 CLI 链，已实现）。 |
